...
import { user_view_latest_appointment_route, view_all_announcements_route } from "../../Routes"
...
export function StudentDashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch(latest_appointment_route, options).then((response) =>{
      if (!response.ok) {
    ...
    fetch(all_announcements_route, options).then((response) =>{
      if (!response.ok) {
        throw Error(`${response.statusText} - ${response.status}`);
      }
      return response.json()
  ...
  const background_switch = () => {
    ...
    return { 
      backgroundImage: `linear-gradient(45deg, rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0)), url(${images[get_random_number(1, 4)]})`,
        ...
        <div className={`${styles.content_container} view_content_layout`}>
          <div className={styles.announcements_container}>
            ...
            {announcements.map((announcement) => {
              return <Announcement className={styles.block_contrast_items_alternative} key={announcement.announcement_id} {...announcement} />;
            })}
          ...
          <div className={styles.appointments_container}>
            ...
            <div className={styles.appointments}>
              ...
              return <AppointmentItem className={styles.block_contrast_items_alternative} key={appointment.appointment_id} {...appointment} />;
              })}
            </div> : <LandingMessage image_disable={true} className={styles.landing_message}>There are no appointments.</LandingMessage>}
          </div>
          <div className={styles.resources_container}>
            ...
            <div className={styles.resources}>
              <ToggleableCard theme="light" className={styles.resources_card} label="Crisis Services">
              <div className="card_information">
                <div className="section">
                  <div>
                    <p>
                      If you are experiencing a crisis, please call Counseling Services at (775) 784-4648 between the hours of 8:00 a.m. – 6:00 p.m. Monday through Thursday and 8:00 a.m. to 5:00 p.m.
                    </p>
                  </div>
                  <div>
                    <p>
                      <span className="bold">Building:</span> William N. Pennington Student Achievement Center
                    </p>
                    <p>
                  ...
                  <div>
                    <p>
                      <span className="bold">After-Hours Crisis Line:</span> (775) 297-8315
                    </p>
                    <p>
                      <span className="bold">Emergency Services:</span> 911
                    </p>
                    <p>
                      <span className="bold">Non-Emergency Police Services:</span> 911
                    </p>
                    <p>
                      <span className="bold">National Suicide Prevention Hotline:</span> (800) 273-8255
                    </p>
                    <p>
                      <span className="bold">Hope Line Network:</span> (800) 784-2433
                    </p>
                  </div>
                </div>
              </div>
              </ToggleableCard>
              <ToggleableCard theme="light" className={styles.resources_card} label="Housing Services">
              <div className="card_information section">
                  <div>
                    <p>
                      Addresses listed below are for receipt of packages by residents during the academic year. Most front desks are closed during winter and summer breaks so packages may be "returned to sender" during these periods. 
                    </p>
                    </div>
                    <div>
                      <p className="bold">
